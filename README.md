...
To build a Slackware64-14.2 full image:
    ...
    $ docker run --privileged --rm -e RELEASENAME=slackware -e ARCH=x86_64 -e VERSION=14.2 -e CHOWN_TO="$(id -u):$(id -g)" -v "$(pwd):/data" -v "$(pwd)/scripts:/scripts" alpine:3.15 sh /scripts/build_base_image.sh
...
To build a Slackware64-15.0 full image:
    ...
    $ docker run --privileged --rm -e RELEASENAME=slackware -e ARCH=x86_64 -e VERSION=15.0 -e CHOWN_TO="$(id -u):$(id -g)" -v "$(pwd):/data" -v "$(pwd)/scripts:/scripts" alpine:3.15 sh /scripts/build_base_image.sh
...
To build a Slackware64-current full image:
    ...
    $ docker run --privileged --rm -e RELEASENAME=slackware -e ARCH=x86_64 -e VERSION=current -e CHOWN_TO="$(id -u):$(id -g)" -v "$(pwd):/data" -v "$(pwd)/scripts:/scripts" alpine:3.15 sh /scripts/build_base_image.sh
